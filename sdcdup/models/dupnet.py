...
def load_checkpoint(filepath, model_basename='dup_model'):
...
class DupCNN(nn.Module):
    ...
    def forward(self, x):
        ...
        x = self.relu(x)  # Computes the activation of the first fully connected layer
...
class DupCNN2(nn.Module):
    ...
    def __init__(self, input_shape, output_size, conv_layers, fc_layers):
        ...
        self.pool = nn.MaxPool2d(kernel_size=2)
        # 8192 input features, 256 output features (see sizing flow below)
    ...
    def forward(self, x):
        ...
        x = self.conv6(x)  # Size changes from (256, 8, 8) to (512, 8, 8)
        x = self.relu(x)  # Computes the activation of the fifth convolution
        x = self.pool(x)  # Size changes from (512, 8, 8) to (512, 4, 4)

        # Reshape data to input to the input layer of the neural net
        # Recall that the -1 infers this dimension from the other given dimension
        x = x.view(-1, 512 * 4 * 4)  # Size changes from (512, 4, 4) to (1, 8192)
